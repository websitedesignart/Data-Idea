...
4. Parse the JSON response:
   ...
   - Key values in the output (`top_groups`, `sample_only_*`) are masked: 12-letter tokens, the same value always giving the same token in a project. The output says `"values": "masked"`. Refer to groups by token and count. Never try to recover, guess or ask the user to paste the underlying identifiers; if a person needs the rows, point them to the evidence links (`finding_id`). `--reveal-values` shows raw values only for a column and values that do not look sensitive, and only if the user asks for it; if the output says `reveal_refused_*`, report that and do not work around it. `"values": "withheld"` means no masking key was available: report it.
